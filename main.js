...
if (firebaseStatus) {
  firebaseStatus.textContent = firebaseReady
    ? 'تم ربط Firebase — النتائج ستُحفظ أونلاين'
    : 'لم يتم ربط Firebase بعد — سيتم استخدام التخزين المحلي للتجربة';
}

if (storageStatus) {
  storageStatus.textContent = firebaseReady
...
function renderBoard() {
  ...
  if (top3List) {
    for (let i = 0; i < 3; i++) {
      ...
      card.innerHTML = item
        ? `<div class="rank-num">${i + 1}</div><div class="rank-name">${item.name}</div><div class="rank-meta">${item.classroom}</div><div class="rank-meta">${item.totalScore} نقطة</div>`
        : `<div class="rank-num">${i + 1}</div><div class="rank-name">---</div><div class="rank-meta">لا يوجد</div>`;
...
function rangeKeys(prefix, from, to, digits = 4) {
  ...
  for (let i = from; i <= to; i++) {
    arr.push(`${prefix}${String(i).padStart(digits, '0')}`);
  }
  return arr;
}
...
class AnimatedArcadeImage extends Phaser.Physics.Arcade.Image {
  ...
  setFrames(frames, restart = false) {
    ...
    if (this.framesList?.length) {
      this.setTexture(this.framesList[Math.min(this.frameIndex, this.framesList.length - 1)]);
    }
  }
...
class BootScene extends Phaser.Scene {
  ...
  preload() {
    ...
    for (let i = 1; i <= 48; i++) {
      this.load.image(`hero${String(i).padStart(4, '0')}`, `assets/hero/hero${String(i).padStart(4, '0')}.png`);
    }

    for (let i = 1; i <= 69; i++) {
      this.load.image(`enemy${String(i).padStart(4, '0')}`, `assets/enemy/enemy${String(i).padStart(4, '0')}.png`);
    }

    for (let i = 1; i <= 12; i++) {
      this.load.image(`coin${String(i).padStart(4, '0')}`, `assets/coins/coins${String(i).padStart(4, '0')}.png`);
    }

    for (let i = 1; i <= 3; i++) {
      this.load.image(`treasure${String(i).padStart(4, '0')}`, `assets/treasure/treasure${String(i).padStart(4, '0')}.png`);
    }
  }
...
class BaseLevelScene extends Phaser.Scene {
  ...
  create() {
    ...
    state.spawnProtectedUntil = this.time.now + 1800;
  }
  ...
  createPlatforms() {
    ...
    data.forEach((p, index) => {
      this.createPlatformTile(p.x, p.y, p.w, p.t);
      if (index === 0) {
        this.respawnPoint = { x: 150, y: 560 };
      }
    });
  }
  ...
  createPlayer() {
    ...
    this.player.body.setSize(80, 140, true);
  }
  ...
  addEnemy(x, y) {
    ...
    enemy.body.setSize(90, 140, true);
  ...
  createTreasures() {
    const data =
      this.levelNumber === 1
        ? [
            { x: 1400, y: 420, t: 'treasure0001', id: 't1' },
            { x: 2700, y: 540, t: 'treasure0002', id: 't2' }
          ]
        : [
            { x: 1730, y: 360, t: 'treasure0003', id: 't3' },
            { x: 2440, y: 300, t: 'treasure0001', id: 't4' }
          ];
  ...
  async triggerHazard(hazard) {
    if (this.challengeActive || this.time.now < state.spawnProtectedUntil) return;

    this.challengeActive = true;
    this.physics.pause();

    const result = await askQuestion('hazard', { level: this.levelNumber, hazard: hazard?.name || 'danger' });
  ...
  async triggerTreasure(treasure) {
    if (
      ...
      this.collectedTreasureIds.has(treasure.treasureId) ||
      this.time.now < state.spawnProtectedUntil
    ) {
      return;
    }
  ...
  respawnPlayer() {
    ...
    state.spawnProtectedUntil = this.time.now + 1200;
  }
...
if (refreshBoardBtn) refreshBoardBtn.addEventListener('click', renderBoard);
if (playAgainBtn) {
